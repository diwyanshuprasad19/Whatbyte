function printGraphs(response, graph_type){
    ...
    switch(graph_type){
        case 'cervix_diameter':
            cervixGraph('#print-cervix-graph', cervix, [], [], tickPositions);
            break;
        case 'head_descent':
            descentGraph('#print-descent-graph', descent, [], [], tickPositions);
            break;    
        case 'blood_pressure':
            bpGraph('#print-bp-graph', systolic_bp, diastolic_bp, tickPositions);
            break;
        case 'pulse_rate':
            pulseGraph('#print-pulse-graph', pulse, tickPositions);
            break;
        case 'fetal_heart_rate':
            fetalGraph('#print-fetal-graph', fetal, tickPositions);
            break;
        case 'amniotic_fluid':
            amnioticGraph('#print-amniotic-graph', amniotic_data, tickPositions);
            break;

        case 'moulding':
            mouldingGraph('#print-moulding-graph', moulding_data, tickPositions);
            break;

        case 'contraction':
            contractionsGraph('#print-contractions-graph', contractions_data, contractions_intensity, tickPositions);
            break;
        case 'oxytocin':
            oxytocinGraph('#print-oxytocin-graph', oxytocin_data, tickPositions);
            break;
        case 'drugs':
            drugsGraph('#print-drugs-graph', drugs_data, tickPositions);
            break;
        case 'temperature':
            temperatureGraph('#print-temperature-graph', temperature, hourTickPostions);
            break;
        ...
        case 'all':
            ...
            //Draw Parto Graph
            if(cervix.length > 0){ cervixGraph('#print-cervix-graph', cervix, [], [], tickPositions);}
            if(descent.length > 0){ descentGraph('#print-descent-graph', descent, [], [], tickPositions);}
            //Draw BP Graph
            if(systolic_bp.length > 0){ bpGraph('#print-bp-graph', systolic_bp, diastolic_bp, tickPositions); }
            //Draw Pulse Graph
            if(pulse.length > 0){ pulseGraph('#print-pulse-graph', pulse, tickPositions); }
            //Draw Fetal Graph
            if(fetal.length > 0){ fetalGraph('#print-fetal-graph', fetal, tickPositions); }
            //Draw Amniotic and Moulding Graph
            if(amniotic_moulding.length > 0){ amnioticMouldingGraph('#print-amniotic-moulding-graph', amniotic_moulding, tickPositions); }
            if(contractions_data.length > 0){ contractionsGraph('#print-contractions-graph', contractions_data, contractions_intensity, tickPositions); }
            if(oxytocin_data.length > 0){ oxytocinGraph('#print-oxytocin-graph', oxytocin_data, tickPositions); }
            if(drugs_data.length > 0){ drugsGraph('#print-drugs-graph', drugs_data, tickPositions); }
            if(temperature.length > 0){ temperatureGraph('#print-temperature-graph', temperature, hourTickPostions); }
            break;
    }
}